...
@section('content')
    <section class="content-header">
        <h1>Edit page <small>Edit an existing page</small></h1>
        <ol class="breadcrumb">
    ...
    <form action="{{ action('Admin\PageController@update', ['id' => $page->id]) }}" method="post">
        <input type="hidden" name="_method" value="PUT">
        {!! csrf_field() !!}
        <section class="content">
            <div class="row">
                <div class="col-md-8">
                    {!! AdminHelper::textField_class("Title", "title", "input-lg", old("title", $page->title), $errors) !!}
                    <div class="slug-info{!! $errors->has("slug") ? ' has-error' : '' !!}">
                        <div id="slug-edit">
                            Slug: <span class="form-inline"><input type="text" id="slug" name="slug" value="{{ old("slug", $page->slug) }}" class="form-control"></span> <button type="button" id="btn-slug-reset" class="btn btn-default btn-sm">Use default</button>
                    ...
                    <div class="form-group{!! $errors->has("body") ? ' has-error' : '' !!}">
                        ...
                        <textarea class="form-control" rows="20" name="body" id="body" data-editor="markdown">{!! old("body", $page->body) !!}</textarea>
                    </div>
                    <div class="form-group{!! $errors->has("data") ? ' has-error' : '' !!}">
                        <label>Data (JSON)</label>
                        {!! $errors->has("data") ? "<span class=\"help-block\"><strong>" . $errors->first("data") . "</strong></span>" : "" !!}
                        <textarea class="form-control" rows="10" name="data" id="data" data-editor="json">{!! old("data", $page->data) !!}</textarea>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="box box-primary">
                        ...
                        <div class="box-body">
                            <?php $published_on_val = old("published_on", $page->published_on); ?>
...
HDC
                    ...
                    <div class="box box-info">
                        ...
                        <div class="box-body">
                            {!! \App\Helpers\AdminHelper::form_group()->title("Template")->field("template")->desc("Specify the template used for this page.")->errors($errors)->value($page->template) !!}
                        </div>
                    </div>
                </div>
            </div>
